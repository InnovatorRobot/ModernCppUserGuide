...
void BoxContainer<T>::expand(size_t new_capacity)
{
    ...
    if (new_capacity <= m_capacity) return; // The needed capacity is already there
...
bool BoxContainer<T>::remove_item(const T& item)
{
    ...
    if (index > m_size) return false; // Item not found in our box here
...
void BoxContainer<T>::operator+=(const BoxContainer<T>& operand)
{
    ...
    if ((m_size + operand.size()) > m_capacity) expand(m_size + operand.size());
